...
-- Colors
local red_clr = "#F53D68" -- "#e54c62"
local green_clr = "#49A762"
local blue_clr = "#7788AF"
local yellow_clr = "#FFD340"
local pink_clr = "#CF85AF"
local orange_clr = "#de5e1e"
local purple_clr = "#B857A6"

-- Textclock
os.setlocale(os.getenv("LANG")) -- to localize the clock
local clockicon = wibox.widget.imagebox(theme.widget_clock)
local mytextclock = wibox.widget.textclock(markup(blue_clr, "%A %d %B ") .. markup("#535f7a", ">") .. markup(orange_clr, " %H:%M "))
...
local cpu = lain.widget.cpu({
    settings = function()
        widget:set_markup(
	    markup.fontfg(theme.font, red_clr, string.format("%-4s", cpu_now.usage .. "%"))
	)
    end
})
...
local bat = lain.widget.bat({
	...
	local color = purple_clr
        ...
        widget:set_markup(markup.fontfg(theme.font, color, string.format("%s%-4s ", baticon, perc)))
    end
})
...
local netupinfo = lain.widget.net({
	...
	--]]
        ...
        widget:set_markup(
	    markup.fontfg(theme.font, red_clr, string.format("%-7s ", net_now.sent))
	)
        netdowninfo:set_markup(
	    markup.fontfg(theme.font, green_clr, string.format("%-7s ", net_now.received))
	)
    end
})
...
local memory = lain.widget.mem({
    settings = function()
        widget:set_markup(markup.fontfg(theme.font, yellow_clr, mem_now.used .. "M "))
    end
})

-- Keyboard map indicator and switcher
local keyboardlayouticon = "⌨"
